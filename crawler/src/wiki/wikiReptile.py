...
class WikiReptile(object):
    ...
    def save_list(self, key_url, start_urls, deep_count=0):
        ...
        for url in start_urls:
            ...
            string = ulb.url2pathname(string)
            # print('\t%s 保存关系 %s' % (str(count), string))
            self.__apend_relation(self.__relationPath, string)
            if url not in self.__urlsList:
                success = self.__apend_html("https://zh.wikipedia.org%s" % url, self.__contextPath)
                if not success:
                    continue
                self.__urlsList.add(url)
                url_List = self.__get_urls("https://zh.wikipedia.org%s" % url)
    ...
    def __get_urls(self, init_url):
        ...
        for link in content.findAll("a", href=re.compile("^(/wiki/)((?!:).)*$")):
            title_len = len(link.attrs['title'])
...
if __name__ == '__main__':
    # 爬取英文维基百科https://zh.wikipedia.org网页内容
    # baseKey = urlencode('/wiki/陈独秀')
    baseKey = ulb.pathname2url('/wiki/陈独秀')
    webDeep = 1
    saveUrl = "../data"
